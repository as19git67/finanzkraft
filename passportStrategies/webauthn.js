...
// NOTE: Type-only re-exports are removed in pure JS.
// export { ChallengeStore, WebAuthnUser, UserStore } from "./types";

export class Webauthn extends PassportStrategy {
  name = 'webauthn';
   ...
   */
  async getUser(identifier, byID = false) {
  ...
  async registerChallenge(req, email) {
  ...
  async registerCallback(req, email, credential) {
  ...
  async loginChallenge(req, email) {
  ...
  async loginCallback(req, email, credential) {
   ...
   */
  async handleRequestByMethod(req, mode, email) {
    if (req.method === 'GET') {
