...
const DbMixinAccounts = {
  ...
  async getAccounts() {
    return this.knex
      ...
      .select([
        ...
        'Fk_Account.fintsAuthRequired',
        'Fk_Account.fintsActivated',
  ...
  async getAccountsWithStatusForUser(userId) {
    return this.knex
      ...
      .leftJoin(
        this.knex
          .select(['idUser', 'Fk_Transaction.idAccount']).count('Fk_Transaction.idAccount as unseen')
          .from('Fk_TransactionStatus').as('FTS')
          .join('Fk_Transaction', function () {
            this.on('Fk_Transaction.id', '=', 'idTransaction');
          })
          .where('idUser', userId)
          .groupBy(['idUser', 'Fk_Transaction.idAccount'])
          .as('status'),
        function () {
